...
*   **상품 관리 엔진 (v124.7x)**:
    ...
    *   **선택 프로세스**: 카테고리 선행 선택 후에만 상품 추가 드롭다운(`isDisabled`) 활성화. (읽기 전용 시에는 `TeasyInput`으로 병합 노출)
*   **결제 및 증빙 섹션**:
    *   **배경 그룹화**: 결제 매출 상세 영역은 `white` 배경 박스 내 배치 (데이터 표현 일관성).
...
*   **플레이스홀더 및 규격 (Verbatim)**:
    ...
    *   **읽기 전용 (Read-Only)**: '담당자', '구매 상품', '결제 방식', '할인 내역', '배송 업체' 등 모든 선택형 필드는 `isReadOnly` 시 `TeasyInput`을 사용하여 데이터를 정갈하게 노출함.
    *   **그룹 박스 배경**: 상품 리스트, 결제 상세, 배송 정보 등 모든 그룹화 박스의 배경은 `white`를 기본으로 함.
...
| v1.3 | 2026-02-03 | 읽기 전용 선택 필드 TeasyInput 전환 및 모든 그룹 박스 white 배경 표준화 |
