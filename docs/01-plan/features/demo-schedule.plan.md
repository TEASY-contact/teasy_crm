...
*   **그리드 레이아웃**:
    *   **2열 그룹화 (HStack)**: [시연 일시-담당자] 항목은 상단에, [연락처-상품] 항목은 하단에 `HStack spacing={4}`로 배치함.
    *   **주소 필드 (Block)**: 방문 주소는 전용 `TeasyInput`으로 단독 행 배치.
...
*   **상태 피드백**: 저장 시 `Spinner`가 적용된 로딩 오버레이 노출.
...
*   **제약 사항**:
    *   시연 일시는 **과거 시점 선택 불가** (`limitType="past"` 설정, 작성 시점 기준 과거 날짜 선택을 차단함).
    *   모든 필수 항목(일시, 담당자, 주소, 연락처, 상품) 기재 필수.
