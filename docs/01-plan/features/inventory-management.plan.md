...
*   **Row Reordering (Manual Sort)**: 
    *   **Drag & Drop**: 테이블 행의 순서를 드래그 앤 드롭으로 자유롭게 변경 가능함 (`framer-motion` Reorder).
    *   **Optimistic UI**: 클라이언트에서 즉시 순서를 반영하고, 백그라운드에서 배치(Batch) 처리로 Firestore `orderIndex`를 갱신함.
    *   **Rollback**: 서버 저장 실패 시 이전 순서로 자동 원복되는 트랜잭션 안전 장치를 포함함.
    *   **New Item Priority**: `orderIndex`가 없는 신규 항목은 무조건 리스트 최상단에 배치됨.
...
*   **Strict Master Data Selection (Meta-Data Enforced)**: 
    *   **자유 입력 금지**: 장부의 무결성을 위해 물품명은 `inventory_master` 컬렉션에 등록된 항목 중에서만 **선택(Select)** 가능함.
    *   **Dropdown UI**: `CustomSelect` 컴포넌트를 사용하여 등록된 마스터 데이터(물품명 `[배송]`) 목록을 제공함.
    *   **동적 필터링**: 카테고리 선택 시 해당 카테고리에 속한 마스터 아이템만 필터링되어 노출됨을 원칙으로 하되, 현재는 전체 목록에서 선택 시 카테고리가 자동 동기화되는 방식을 사용함.
...
### 3.4 마스터 데이터 관리 (Inventory Master Management)
*   **목적**: 재고 물품명의 표준화 및 배송 여부(Shipping Status) 속성을 관리하는 메타 데이터 제어 센터.
*   **진입점**: 재고 관리 모드 상단의 **`[설정] 물품 등록`** 버튼을 통해 전용 모달(`InventoryMasterModal`) 진입.
*   **섹션 분리 (Split View)**:
    *   **배송 정보 포함 (ON)**: 배송이 필요한 주요 자재. 리스트 상단에 배치됨.
    *   **배송 정보 미포함 (OFF)**: 내부 소모품 등 배송과 무관한 자재. 하단에 별도 그룹으로 배치됨.
    *   **Placeholder**: 각 섹션에 데이터가 없을 경우 전용 안내 문구를 노출함.
*   **CRUD 및 제어**:
    *   **추가(Add)**: 카테고리 및 물품명 입력 후 등록. 중복 이름은 등록 차단됨. 신규 등록 시 기본적으로 **'배송 정보 포함'** 상태로 생성됨.
    *   **삭제(Remove)**: 리스트 우측의 삭제 아이콘을 통해 즉시 제거.
    *   **상태 토글(Toggle)**: '배송' 스위치(Switch)를 통해 ON/OFF 상태를 즉시 변경하며, 이에 따라 섹션 간 이동이 발생함.
    *   **순서 변경(Reorder)**: 드래그 핸들(`MdDragHandle`)을 통해 섹션 내부에서 순서를 자유롭게 조정 가능하며, 변경 즉시 Firestore에 반영됨.
*   **시각적 요소**:
    *   **자동 순번**: 리스트 내 위치에 따라 원문자(①, ②...)가 자동 부여됨 (`getCircledNumber`).
    *   **카테고리 배지**: 물품명 좌측에 `gray` 타입의 카테고리 배지를 노출하여 식별력을 높임.
...
| v1.85 | 2026-02-02 | **마스터 데이터 관리(3.4) 섹션 추가 및 물품명 선택(Strict Selection) 로직 동기화 완결** |
