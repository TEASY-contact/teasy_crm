// src/components/features/admin/UserEditModal.tsx
"use client";
import { useState, useRef } from "react";
import {
    VStack, FormControl, useToast, Divider, Text, Switch, Flex,
    Alert, AlertIcon, AlertTitle, AlertDescription, Box, Grid, HStack, IconButton, Center
...
const ALL_CANDIDATE_COLORS = [
    ...
    "#e36414", "#0f4c5c", "#00b4d8", "#0077b6", "#023e8a", "#03045e", "#ff4d6d", "#c9184a", "#ff7f50", "#ff8fa3",
...
/**
 * Internal content component for editing user data.
 * Automatically resets on close thanks to TeasyModal's unmounting.
 */
const UserEditModalContent = ({ onClose, user, existingUsers = [], hideAdminFields = false }: Omit<UserEditModalProps, 'isOpen'>) => {
    ...
    const usedColors = existingUsers
        .filter((u: UserData) => u.status !== 'banned' && u.uid !== user.uid)
    ...
    const [formData, setFormData] = useState({
        ...
        email: user.email,
        role: user.role,
        color: user.representativeColor,
        canAccessCustomerData: user.canAccessCustomerData,
        kakaoWorkEnabled: user.kakaoWorkEnabled,
    ...
    const handleSave = async () => {
        ...
        const isColorTaken = existingUsers
            .filter(u => u.status !== 'banned' && u.uid !== user.uid)
        ...
        setIsLoading(true);
        try {
            // 1. Calculate Badge Char if name changed or missing
            ...
            let calculatedBadgeChar = user.badgeChar || formData.name[0] || "?";

            // Re-calculate if name changed or if it's currently a duplicate with others
            if (formData.name !== user.name || usedBadgeChars.has(calculatedBadgeChar) || !user.badgeChar) {
                const baseChar = formData.name[0] || "?";
                if (!usedBadgeChars.has(baseChar)) {
                    calculatedBadgeChar = baseChar;
                } else if (formData.name.length >= 2 && !usedBadgeChars.has(formData.name[1])) {
                    ...
                    calculatedBadgeChar = `${baseChar}2`;
                }
            }
            ...
            if (formData.newPassword) {
                // actual password update would require auth session/function
            }
    ...
    const handleDelete = async () => {
        if (!window.confirm("정말로 이 계정을 삭제하시겠습니까? 계정 정보가 시스템에서 완전히 제거됩니다.")) return;
                ...
                <VStack spacing={6} align="stretch">
                    {formData.isBanned && (
                        <Alert status="error" borderRadius="md" variant="subtle">
                            <AlertIcon as={MdBlock} />
                            <Box flex="1">
                                <AlertTitle fontSize="sm">활동 정지된 계정입니다</AlertTitle>
                                <AlertDescription fontSize="xs">해당 사용자는 로그인이 불가능합니다.</AlertDescription>
                            </Box>
                        </Alert>
                    )}
                    ...
                    <VStack spacing={4} align="stretch">
                        <FormControl isRequired>
                            ...
                            <TeasyInput
                                ...
                                isDisabled={isBanned || currentUserData?.role !== 'master'}
                            />
                        </FormControl>

                        <FormControl isRequired>
                            <TeasyFormLabel>이메일 (아이디)</TeasyFormLabel>
                            <TeasyInput type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} isDisabled={isBanned || hideAdminFields} />
                        </FormControl>

                        {!hideAdminFields && (
                            <>
                                <FormControl>
                                    <TeasyFormLabel>새 비밀번호 (변경 시에만)</TeasyFormLabel>
                                    <TeasyInput type="password" value={formData.newPassword} onChange={(e) => setFormData({ ...formData, newPassword: e.target.value })} isDisabled={isBanned} placeholder="입력" />
                                </FormControl>
                                ...
                                <FormControl isRequired>
                                    ...
                                    <CustomSelect
                                        ...
                                        isDisabled={isBanned || user.role === 'master'}
                                        placeholder="선택"
                                        options={[
                            ...
                            <>
                                <FormControl isRequired pt={2}>
                                    <Flex justify="space-between" align="center">
                                        ...
                                        <Switch size="md" colorScheme="brand" isChecked={formData.kakaoWorkEnabled} onChange={(e) => setFormData({ ...formData, kakaoWorkEnabled: e.target.checked })} isDisabled={isBanned} />
                                    </Flex>
                                </FormControl>

                                <FormControl isRequired>
                                    <Flex justify="space-between" align="center">
                                        <TeasyFormLabel mb={0}>고객 데이터 검색</TeasyFormLabel>
                                        <Switch size="md" colorScheme="brand" isChecked={formData.canAccessCustomerData} onChange={(e) => setFormData({ ...formData, canAccessCustomerData: e.target.checked })} isDisabled={isBanned} />
                                    </Flex>
                                </FormControl>
                            </>
                        )}

                        {!hideAdminFields && user.role !== 'master' && (
                            <FormControl isRequired>
                                <Flex justify="space-between" align="center">
                                    <Box>
                                        <TeasyFormLabel mb={0} color={formData.isBanned ? "red.500" : "gray.700"}>퇴사 처리</TeasyFormLabel>
                                        <TeasyFormHelperText>계정 정지 및 기록 보존</TeasyFormHelperText>
                                    </Box>
                                    <Switch size="md" colorScheme="red" isChecked={formData.isBanned} onChange={(e) => setFormData({ ...formData, isBanned: e.target.checked })} />
                                </Flex>
                            </FormControl>
                        )}
                    </VStack>
...
export const UserEditModal = (props: UserEditModalProps) => {
    return (
        <TeasyModal isOpen={props.isOpen} onClose={props.onClose} size="lg">
            <TeasyModalOverlay />
            <UserEditModalContent {...props} />
        </TeasyModal>
    );
};
